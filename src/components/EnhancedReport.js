...
class EnhancedReport extends Component {
  ...
  parseCSV = (csvText) => {
    ...
    Object.assign(t, this.state.report2);

    t.setSections(reportSections)
    t.setHeaders(headers.filter(x => x.length > 1))

    this.setState({ report2: t })
  ...
  render() {
    ...
    const { expandAll, report2, isLoading, columnWidths, report, total, title, headers, parameters, date, dividers, reportType } = this.state;
    return (
      <div style={reportStyle}>
        {isLoading ? (
          ...
          <>
            <button onClick={() => this.expandAll()} style={buttonStyle}>
              {this.state.expandAll === false ? 'Expand All' : 'Collapse All'}
            </button>
              ...
              <div style={table_area}>
                {reportType[0] === 'list' ?
                  ...
                  (this.state.printerFriendly === false ?
                    ...
                    // <EnhancedReportTable report={report2}/>
                    <Tester report={report2} expandAll={expandAll} />
                    :
